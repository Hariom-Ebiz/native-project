...
const AccountActionDropdown = () => {
  ...
  const { loggedIn, firstName, lastName, profilePic,companyProfile, role } = useSelector(
    (store) => store.auth
  );
  ...
  return (
    <div className="btn-group company_coinbase_box user_dropdown dash_dropdown">
      ...
      >
        ...
        ) : firstName && companyProfile.logo ? (
          <>
            <span className="user_img">
              <img
                src={`${API}/${companyProfile.logo}`}
                alt=""
                style={{ width: "40px", height: "40px" }}
              />
              {firstName}
      ...
      <ul className="dropdown-menu" aria-labelledby="dropdownMenuButton">
        ...
        <li>
          <Link className="dropdown-item" href="/employer" onClick={()=>{
            dispatch(logout())
          }}>
            <span className="dropitem_icon">
              <LogOut />
