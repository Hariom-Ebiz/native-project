...
const Course = ({ categories, progress, thirdCompleted,course_package_details }) => {
  ...
  const pushToLessonsListingHandler = (
    ...
    router.push(`/standout/chapter/lesson/${courseId}`);
    
    const pushHandler = () => {
      
      if (skip_lessons) {
        router.push(`/standout/chapter/lesson/description/${lessonId}`);
        // router.push(
        //   {
        //     pathname: `/standout/lesson/description/${lessonId}`,
        //     query: {
        //       id: categoryId,
        //     },
        //   },
        //   `/standout/lesson/description/${lessonId}`
        // );
      } else {
        router.push(`/standout/chapter/lesson/${courseId}`);
      }
  ...
  return (
    <JobSeekerAuth data={{ title: course_package_details.title,backArrow:"/standout?query=true" }} >
                ...
                <div key={category.id}>
                  ...
                  <div className="row g-4">
                    {category.courses.map((course) => {
                      const totalSeconds = course?.standout_lessons?.reduce((sum, l)=> {
      ...
      >
          ...
          <Modal.Body>
              <div className={style.modal_content} style={{ textAlign: "left" }}>
                  <p dangerouslySetInnerHTML={{__html: course_package_details.description}}></p>
              </div>
          </Modal.Body>
      </Modal>
    </JobSeekerAuth>
