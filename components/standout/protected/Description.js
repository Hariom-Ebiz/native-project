...
const Description = ({ lesson }) => {
  ...
  const updateLessonProgressHandler = (playedSeconds) => {
    request("POST", "standout/lesson-progress", {
      id: progressId,
      lesson_id: lesson.id,
  ...
  const completeLessonHandler = () => {
    if (!isCompleted) {
      requestComplete("PUT", "standout/lesson-progress/complete", {
        id: progressId,
        total_seconds: lesson.total_seconds,
        lesson_id: lesson.id,
  ...
  const checkIsThirdCategroyCompletedHandler = () => {
    requestIsThirdCategoryCompleted("GET", "standout/category/third-category");
  };
  ...
  return (
    <JobSeekerAuth
      data={{
        ...
        //     ? "/standout"
        //     : `/standout/lesson/${lesson.course_id}`,
        backArrow: `/standout#${lesson.course_data.category_id}`,
      }}
    >
        ...
        <div className={["main_content main_bg", `${(lang == "AR") ? style.land_ar : ""}`]} id="body_lang_css">
            ...
            <p>
              <Link
                href={
                  lesson.course_data.skip_lessons === 1
                    ? "/standout"
                    : `/standout/chapter/lesson/${lesson.course_id}`
                }
              >
                <svg
                  width="38"
              ...
              <div className={style.complete_btn_top}>
                ...
                {isNextRouteAvailable && (
                  <div className={style.download_pdf}>
                    <Link
                      href={`/standout/chapter/lesson/description/${lesson.next_lesson_id}`}
                    >
                      Next
                    </Link>
                  </div>
          ...
          {/* {lesson.is_assessment == 1 && (
            <AssessmentComponent
              ...
              nextLink={`/standout/lesson/description/${lesson.next_lesson_id}`}
              lessonId={lesson.id}
          ...
          <div className={style.right_btn_block_bottom}>
            {lesson.prev_lesson_id ? (
              <div className={`${style.download_pdf} ${style.prev_btn}`}>
                <Link
                  href={`/standout/chapter/lesson/description/${lesson.prev_lesson_id}`}
                >
                  Previous
                </Link>
              </div>
