...
const Description = ({ lesson }) => {
  ...
  const completeLessonHandler = () => {
    if (!isCompleted) {
      requestComplete("PUT", "lesson-progress/complete", {
        id: progressId,
        total_seconds: lesson.total_seconds,
        lesson_id: lesson.id,
        course_id: lesson.course_id,
  ...
  const checkIsThirdCategroyCompletedHandler = () => {
    requestIsThirdCategoryCompleted("GET", "course-category/third-category");
  };
  ...
  return (
    <JobSeekerAuth
      data={{
        ...
        //     ? "/course"
        //     : `/course/lesson/${lesson.course_id}`,
        backArrow: `/course#${lesson.course_data.category_id}`,
      }}
    >
      <div className="page_container">
        <div className={["main_content main_bg", `${(lang == "AR") ? style.land_ar : ""}`]} id="body_lang_css">
              ...
              <div className={style.complete_btn_top}>
                ...
                {isNextRouteAvailable && (
                  <div className={style.download_pdf}>
                    <Link
                      href={`/course/lesson/description/${lesson.next_lesson_id}`}
                    >
                      Next
                    </Link>
                  </div>
          ...
          {!lesson.document && lesson.is_assessment == 0 && (
            <div className={style.vedio_block}>
              ...
              <ReactPlayer
                ...
                url={lesson.video}
                // url={"https://my-test-pull-zone-native.b-cdn.net/856787-hd_1920_1080_30fps.mp4"}
                controls={true}
                playbackRate={1}
                pip={false}
          ...
          {lesson.is_assessment == 1 && (
            <AssessmentComponent
              ...
              nextLink={`/course/lesson/description/${lesson.next_lesson_id}`}
              lessonId={lesson.id}
          ...
          <div className={style.right_btn_block_bottom}>
            {lesson.prev_lesson_id ? (
              <Link
                  href={`/course/lesson/description/${lesson.prev_lesson_id}`}
                >
              <div className={`${style.download_pdf} ${style.prev_btn} px-4`}>
                  Previous
                  </div>
                </Link>
            ) : (
            ...
            {isNextRouteAvailable && (
              <Link
                href={`/course/lesson/description/${lesson.next_lesson_id}`}
              >
                <div className={`${style.download_pdf} px-4`}>
                  Next
                </div>
              </Link>
            )}
