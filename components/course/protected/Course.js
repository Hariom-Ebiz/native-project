...
const Course = ({ categories, progress, thirdCompleted }) => {
  const {subscription: {career_coaching},is_subscriber}  = useSelector(store => store.auth)
  ...
  const handlePackageStartClick = (categoryId,
    courseId,
    skip_lessons,
    lessonId) => {
    if(career_coaching){
      pushToLessonsListingHandler(categoryId,
        courseId,
        skip_lessons,
        lessonId)
    } else {
                ...
                <div key={category.id}>
                  ...
                  <div className="row g-4">
                    {category.courses.map((course) => {
                      ...
                      const totalSeconds = course?.lessons?.reduce((sum, l)=> {
                              ...
                              <div className={style.course_detais}>
                                ...
                                <div className={style.lesson_time}>
                                  ...
                                  <button
                                    ...
                                    // href={
                                    //   course.skip_lessons === 1
                                    //     ? `course/lesson/description/${course.lessons[0]}`
                                    //     : `course/lesson/${course.id}`
                                    // }
                                    // href="javascript:void(0)"
                                    onClick={() => {
                                      console.log("course.id",category.id,
                                        course.id,
                                        course.skip_lessons === 1,
                                        course.lessons[0].id);
                                      
                                      if (course.lessons.length == 0) return;
                                      handlePackageStartClick(
                                        category.id,
                                        course.id,
                                        course.skip_lessons === 1,
                                        course.lessons[0].id
            ...
            >
                <Modal.Header closeButton>
                    <div className={style.modal_content}>
                        <span style={{ fontSize: "24px", }}>Please Purchase Career Journey Package !</span>
                    </div>
                </Modal.Header>
                <Modal.Body>
                    <div className={style.modal_content} style={{ textAlign: "left" }}>
                        <h6>
                            You have to purchase Career Journey Package before accessing it.
                        </h6>
