...
const Description = ({ lesson }) => {
  const packageId = lesson.course_data.package_id;
  ...
  const updateLessonProgressHandler = (playedSeconds) => {
    request("POST", "functional-mastery/lesson-progress", {
      id: progressId,
      lesson_id: lesson.id,
  ...
  const completeLessonHandler = () => {
    if (!isCompleted) {
      requestComplete("PUT", "functional-mastery/lesson-progress/complete", {
        id: progressId,
        total_seconds: lesson.total_seconds,
        lesson_id: lesson.id,
  ...
  const checkIsThirdCategroyCompletedHandler = () => {
    requestIsThirdCategoryCompleted("GET", "functional-mastery/category/third-category");
  };
  ...
  return (
    <JobSeekerAuth
      data={{
        ...
        //     ? "/functional-mastery"
        //     : `/functional-mastery/lesson/${lesson.course_id}`,
        backArrow: `/functional-mastery#${lesson.course_data.category_id}`,
      }}
    >
        ...
        <div className={["main_content main_bg", `${(lang == "AR") ? style.land_ar : ""}`]} id="body_lang_css">
            ...
            <p>
              <Link
                href={
                  lesson.course_data.skip_lessons === 1
                    ? "/functional-mastery"
                    : `/functional-mastery/chapter/lesson/${lesson.course_id}`
                }
              >
                <svg
                  width="38"
              ...
              <div className={style.complete_btn_top}>
                ...
                {isNextRouteAvailable && (
                  <div className={style.download_pdf}>
                    <Link
                      href={`/functional-mastery/chapter/lesson/description/${lesson.next_lesson_id}`}
                    >
                      Next
                    </Link>
                  </div>
          ...
          {/* {lesson.is_assessment == 1 && (
            <AssessmentComponent
              ...
              nextLink={`/functional-mastery/lesson/description/${lesson.next_lesson_id}`}
              lessonId={lesson.id}
          ...
          <div className={style.right_btn_block_bottom}>
            {lesson.prev_lesson_id ? (
              <div className={`${style.download_pdf} ${style.prev_btn}`}>
                <Link
                  href={`/functional-mastery/chapter/lesson/description/${lesson.prev_lesson_id}`}
                >
                  Previous
                </Link>
              </div>
