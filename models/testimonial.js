...
exports.getAllForClient = async (lang) => {
  return new Promise((resolve, reject) => {
    ...
    dataQuery.select(
      "testimonials.*",
      "testimonial_language.designation as language_designation",
      "testimonial_language.comment as language_comment",
      "testimonial_language.language_id as language_id",
    )
    dataQuery.where({});
    dataQuery.leftJoin("testimonial_language", function() {
      this.on("testimonials.id", "=", "testimonial_language.testimonial_id")
      .andOn("testimonial_language.language_id", "=", DB.raw('?', [lang ?? 0]))
    })
    dataQuery
      .then(function (record) {
        resolve(record);
