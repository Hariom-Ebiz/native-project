...
const StepThree = ({ setData, trackForm, validateStep, coreValues, intrestsData, motivateSkills, personalitySummary, psycometrics }) => {
  ...
  const [interests, setInterests] = useState(intrestsData);
  ...
  const { request: RecommendedReq, response: recommandedResp } = useRequest()
  ...
  useEffect(() => {
    ...
    if (interests_values) {
      const [filterdData, unFilterdData] = intrestsData.reduce((acc, val, index) => {
        if (interests_values.includes(val.id)) {
          acc[0].push({ ...val, id: val.id })
        } else {
          acc[1].push({ ...val, id: val.id })
        }
        return acc;
      }, [[], []])
  ...
  const onSubmit = async (data) => {
    ...
    if (data.interest_weight && data.interest_weight != 0 && (!data.interests_values || data.interests_values.length <= 0)) {
      setError("interests_values", { message: "This field is required." });
      if (careerInterestRef.current) {
  ...
  const checkRemaining = (type, val) => {
    ...
    // const a = (type == "aptitude_weight") ? val : (watch("aptitude_weight") || 0);
  ...
  useEffect(() => {
    if (recommandedResp) {
      if (recommandedResp.list.length) {
        for (const i of recommandedResp.list) {
          setValue(i.slug, i.value);
          clearErrors(i.slug)
        }
      }
    }
    ...
    <>
      ...
      <div className={`${styles.psychomet_scoreDiv} ${styles.psychomet_scoreBlock}`}>
        <div className={styles.screening_assessment_top}>
          ...
          <button className={`post_btn border-0 ${styles.use_default_btn}`} onClick={() => setRecommandedScorePsyco()}>{t("Use")} {t("Default")} {t("Weighting")}</button>
        </div>
                ...
                <div className="row">
                  ...
                  <div className="col-md-6 col-xxl-4">
                    <div className={styles.five_valueDiv}>
                      ...
                      {errors['career_values'] && <><span className="text-danger">{errors['career_values'].message}</span><br /></>}
                  ...
                  <div className={styles.form_InputFlex}>
                    <input type="number"
                      {...register("interest_weight", {
                        ...
                        // validate: {
                        //   remainingWeightCheck: (value) => Number(value) > Number(checkReaminingWeight) || `Weight must be under the remaining weight (${checkReaminingWeight}%).`
                        // }
                ...
                <div className="row">
                  ...
                  <div className="col-md-6 col-xxl-4">
                    <div className={styles.five_valueDiv}>
                      ...
                      {errors['interests_values'] && <><span className="text-danger">{errors['interests_values'].message}</span><br /></>}
            ...
            <div className={`${styles.post_job_left_section} ${styles.weighting_box}`}>
              ...
              <div className={styles.weighting_scoreBox}>
                <div className={styles.form_group}>
                  <label className={styles.form_labelWeighting}>{t("Weighting")} :</label>
                  <div className={styles.form_InputFlex}>
                    {console.log(">>>>>>>>", checkReaminingWeight)}
                    <input type="number"
          ...
          <div className="col-sm-8">
            <div className={styles.choose_careerBlock}>
              ...
              <ul className={styles.commanding_typeStep}>
                {typeof groupedData == "object" && Object.keys(groupedData).map((groupName, groupIndex) => (
      ...
      <div className={`mb-0 mt-4 ${styles.psychomet_scoreDiv}`}>
        <div className={styles.screening_assessment_top}>
          ...
          <button className={`post_btn border-0 ${styles.use_default_btn}`} onClick={() => setRecommandedScore()}>{t("Use")} {t("Default")} {t("Weighting")}</button>
        </div>
      </div>
      <div className={styles.job_details_box}>
        <div className={`row justify-content-center  ${styles.row_gap}`}>
          {/* <div className="col-sm-4">
            <div className={`${styles.post_job_left_section} ${styles.weighting_box}`}>
              ...
              <div className={styles.weighting_scoreBox}>
                <div className={styles.form_group}>
                  ...
                  <div className={styles.form_InputFlex}>
                    <input type="number" 
                     {...register("aptitude_weight",{
                      required: checkReaminingWeight > 0 ? true : false,
                      max: 100,
                    })} 
                    
                    onChange={(e) => handleChange(e, "aptitude_weight")}
                    className={styles.form_InputWeighting} placeholder="20" min="0" />
                    <span className={styles.form_leftWeighting}>{checkReaminingWeight}% left</span>
                  </div>
                  {errors['aptitude_weight'] && <span className="text-danger">This field is required</span>}
                  <span className={styles.form_recommendedText}>Recommended Weight%</span>
                  ...
                  <div>
                    <div className={styles.input_groupflex} style={{ display: "inline-block" }}>
                      <input 
                        ...
                        max={10} 
                        {...register("verbal_number",
                          { required: { 
                              value: true, 
                              message: "This field is required." 
                            }, 
                            max: { 
                              value: (100 - (parseInt(watch("situational_number") || 0) + parseInt(watch("logical_number") || 0) + parseInt(watch("numerical_number") || 0))), 
                              message: "Total Weight must be equal 100%." 
                            } 
                          })} 
                        placeholder="25%" 
