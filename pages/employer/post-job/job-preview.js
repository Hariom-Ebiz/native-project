...
const JobPreview = ({ category, jobType, skills, trackForm }) => {
  ...
  let skillName = Array.isArray(formVals.keywords) ? formVals?.keywords : []

  const [is_saved, seIsSave] = useState(false);
  ...
  const onSubmit = async (data) => {
    ...
    data.is_posted = (isSave) ? 0 : 1;

    PostJobRequest("POST", "employer/job-post", data);
  };
  ...
  useEffect(() => {
    if (postJobResponse) {
      route.push(`/employer/manage-jobs${is_saved ? "?isSaved=true" : ""}`)
    }
  }, [postJobResponse])
    ...
    <>
      <div className={styles.social_head}>
        <div className={styles.left_box}>
          ...
          {
            (!expandTitle) ? <h4 className={styles.social_head_title}>{formVals?.title?.substring(0,40)}{(formVals?.title.lngth > 40) && <span onClick={() => setExpandTitle(true)} style={{cursor: "pointer"}}>...</span>}</h4> : <h4 className={styles.social_head_title}>{formVals.title}</h4>
          }
        </div>

        <div className={` ${styles.post_job_inner_btn} ${styles.next_step_btn_block}`}>
          <button onClick={() => gotoEdit()} className={`${styles.next_btn} ${styles.post_now_btn}`}>
            {t("Edit")}
          </button>
          <button onClick={() => { isSave = true; seIsSave(true); handleSubmit(onSubmit)() }} className={`${styles.next_btn} ${styles.post_now_btn}`}>
            {t("Save")} & {t("Post")} {t("Later")}
          </button>
          ...
          <div className="col-md-7">
            <div className={styles.details_page_left}>
              <div className={styles.details_content_box}>
                <h2 className={styles.box_title}>Job Description</h2>
              ...
              <div className={styles.details_content_box}>
                <h2 className={styles.box_title}>{t("About")} {t("Company")}</h2>
          ...
          <div className="col-md-4">
            <div className={styles.details_page_right}>
              <div className={styles.job_about_box}>
                <h2 class={styles.box_title}>{t("About")} {t("this")} {t("role")}</h2>

                <div>
                  <ul className={styles.role_list}>
                    <li className={styles.role_list_items}>
                      <span className={styles.job_tag}>{t("Apply")} {t("Before")}</span>
                    ...
                    <li className={styles.role_list_items}>
                      <span className={styles.job_tag}>{t("Job")} {t("Type")}</span>
                    ...
                    <li className={styles.role_list_items}>
                      <span className={styles.job_tag}>Salary</span>
              ...
              <div className={styles.job_about_box}>
                <h2 class={styles.box_title}>{t("Skills")}/{t("Keywords")}</h2>
