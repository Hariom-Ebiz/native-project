...
import { faqPageData, getFaqByCategory, searchQuestion, likeOrDislike } from "@/services/faq";
...
const HelpCenter = ({ faqs, faqCategory,lang_id }) => {
  ...
  const getByCategory = async (category_id) => {
    ...
    try {
      const getFaqs = await getFaqByCategory(category_id, 2, lang_id);
    ...
    try {
      ...
      if (e.target.value) {
        if (e.target.value.length > 2) {
          const getVal = await searchQuestion(e.target.value, 2, lang_id);
          setFaqs(getVal.faqs)
  ...
  return (
    <>
      <EmployerAuth data={{ title: "Help Center" }} />
                ...
                <div className={styles.left_box}>
                  ...
                  <div className={styles.left_content_box}>
                    ...
                    <div className={styles.question_categories}>
                      <h3 className={styles.question_categories_title}>{t("Getting Started")}</h3>
...
export async function getServerSideProps(context) {
  ...
  let data = await faqPageData(2, lang_id);
  return {
    props: {
      ...data,
      isProtected: null,
