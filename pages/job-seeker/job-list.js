...
import { jobCategory, jobTypeList, getJobSalaryRange, careerLevelList } from "@/services/master";
import { createAxiosCookies, getCookies } from "@/fn";
import useRequest from "@/hooks/useRequest";
import { getJobSeekerJobList } from "@/services/jobSeeker/jobs";
...
const JobList = ({ Jobs, jobTypeList, jobCategory, salaryRange, careerLevels }) => {
  ...
  const handleRequest = (filter) => {
    ...
    jobDataReq("GET", "employer/job-post/get/job-seeker/jobs?" + queryString)
  }
  ...
  useEffect(() => {
    if (aplyRes) {
      ...
      setJobList(prevJobs => {
        return prevJobs.filter(j => j.id != aplyRes.jobId)
      })
  ...
  return (
    <JobSeekerAuth data={{ title: "Find Jobs" }}>
                ...
                <div className={`col-lg-4 col-md-6 ${styles.firstCol}`} style={{ width: "39%" }}>
                  <div className={styles.jobSearchInput}>
                    ...
                    <input
                      ...
                      type="text"
                      placeholder={`${t("Search Jobs by Job Title")}`}
                      value={title}
            ...
            <div className={`row ${styles.rowGap}`}>
              <div className=" col-xxl-3 col-lg-4 ">
                <div
                  ...
                  id="filter-show"
                >
                  <div className="accordion" id="select_categoris_id">
                    <div
                    ...
                    >
                      ...
                      >
                        <div className="accordion-body p-0">
                          ...
                          >
                            <input type="checkbox" name=""
                              checked={(jobCategoryFilter.includes(0))}
                              onChange={(e) => handleChange(e, "job_category", 0)} id={`category_${0}`} />
                            <label htmlFor={`category_${0}`}>{t("Others")} ({jobCategory.find(jc => jc.id == 0).count})</label>
                          </div> */}
                        </div>
                      </div>
                    </div>
              ...
              <div className="col-xxl-9 col-lg-8">
                <div className="showResumeBox">
                  <div className={styles.showResumeHeader}>
                    <div className="showResumeLeft">
                      <h2 className={styles.all_cv_title}>{t("All Jobs")}</h2>
                      <p className={styles.showing_results_text}>
                        {/* {jobsList.length == 0 ? "No matching jobs found." :  pageChange == "1" ?
                            `Showing 1 to ${jobsList.length} of ${totalDoc} results`
                            :
                            `Showing ${(PER_PAGE * (pageChange - 1)) + 1} to ${(PER_PAGE * (pageChange - 1)) + jobsList.length} of ${totalDoc} results`} */}
                        {jobsList.length > 0 ? pageChange == "1" ?
                          t("showText1InJobList", { 1: 1, 2: jobsList.length, 3: totalDoc })
                          :
                          t("showText1InJobList", { 1: (PER_PAGE * (pageChange - 1)) + 1, 2: (PER_PAGE * (pageChange - 1)) + jobsList.length, 3: totalDoc })
                          : ""}
                      </p>
                    </div>
                ...
                </div>
                {
                  jobsList?.map(d => {
                    return (
                      <div className={styles.job_relevant_Box} key={d.id}>
                        <div className="row">
                            ...
                            <div className={styles.job_relevant_left}>
                              ...
                              <div className={styles.cv_content}>
                                <div className={styles.userNameBox}>
                                  <Link
                                    href={`/job-seeker/job-description/${d.id}?from=job-list`}
                                  >
                                    <h3 className={styles.user_name_text} style={{ wordBreak: "break-word" }}>{d.title.substring(0, 80)}</h3>
                                  </Link>
                                </div>
                                ...
                                {!d.hide_salary_range && <div style={{ color: "#7C8493", fontSize: "14px", fontWeight: 400, marginBottom: "10px" }}>
                                  {t("Salary")} : {kFormatter(d.salary_range_from)} {String(d.salary_currency).toUpperCase()} to {kFormatter(d.salary_range_to)} {String(d.salary_currency).toUpperCase()}
                                </div>}
                                ...
                                <ul className={styles.statusBox}>
                                  ...
                                  >
                                    {d.job_category_id ? t(d.job_category_name) : d.other_job_category}
                                  </li>
                                </ul>
                              </div>
                            </div>
                          ...
                          <div className="col-xl-4 col-md-4 col-lg-12">
                            <div className={styles.job_relevant_right}>
                              ...
                              <div className="mt-2" style={{ color: "#7C8493", fontSize: "14px", fontWeight: 400 }}>
                                {t("Posted on")} : {moment(d.posted_on).format("MMMM DD, YYYY")}
                              </div>
                ...
                {jobsList.length > 0 ? <div className={styles.paginationBox}>
                  <Pagination>
                ...
                </div> : <p>{t("No Matching Jobs Found!")}</p>}
      ...
      >
        ...
        <Modal.Body>
          <div className={styles.modal_content} style={{ textAlign: "left" }}>
            <p>
              {t("Search through diverse opportunities from various employers and leverage our advanced filtration criteria to find and apply for the ideal match for your career aspirations.")}
            </p>
          </div>
        </Modal.Body>
      </Modal>
