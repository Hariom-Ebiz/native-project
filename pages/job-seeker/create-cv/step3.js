...
const Step3 = ({
  ...
  const onSubmit = (data) => {
    ...
    jobArray.forEach((uuid) => {
      if (data[`is_currently_working_${uuid}`]) {
          if(checkurrentWorking == 0) {
            checkurrentWorking = checkurrentWorking+1;
          } else if(checkurrentWorking == 1) {
            hasError = true;
            setError(`is_currently_working_${uuid}`, {
              type: "manual",
              message: "Currently working can only be one.",
            });
            scrollToThis(`is_currently_working_${uuid}`);
