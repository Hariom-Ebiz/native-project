...
import NotificationSetting from "@/components/jobSeeker/NotificationSetting";
...
const EditProfile = ({ profileData, statusData, liveInData }) => {
  ...
  useEffect(() => {
    setImage(details && details.profile_picture ? details.profile_picture : "");
    setValue("first_name", profile?.first_name);
    setValue("last_name", profile?.last_name);
    setValue("email", profile?.email);
    setValue("gender", details?.gender);
    setValue("status", details?.status);
    setValue("lives_in", details?.lives_in);
    setValue("yob", details?.yob);
  }, [profile, details]);
