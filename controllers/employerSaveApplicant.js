...
exports.getAll = async (req, res, next) => {
  ...
  try {
    // Retrieve the applicants based on the search query and whether profiles are locked
    data = await SaveApplicant.getMany(
      {
        page,
        per_page,
    ...
    const applicants = data?.data.filter(applicant => {
      if (applicant.is_locked) {
        const newFullName = applicant.first_name + " " + applicant.last_name[0];
        if (search && !newFullName.toLowerCase().includes(search.toLowerCase())) {
          return null;
        }
      }
      return applicant;
