...
const standOutModal = require("../models/standout");
...
exports.getAllGroupByCategories = async (req, res, next) => {
    ...
    try {
        const resp = await standOutModal.getAllGroupByCategories(currentUserId);
...
exports.getAllStandoutCourse = async (req, res, next) => {
    ...
    try {
        list = await standOutModal.getAllCourses({
            query: {...extras},
            page,
            per_page,
            sort_by,
...
exports.getAllStandoutCourseFront = async (req,res, next) => {
    ...
    try {
        list = await standOutModal.getAllCoursesFront({
            match: {},
            page,
            per_page,
            sort_by,
...
exports.deleteStandoutCourse = async (req,res,next) =>{
    ...
    try {
        await standOutModal.deleteCourse({ id });
        await standOutModal.deleteLesson({ "course_id":id });
    } catch (err) {
        console.log("err : ",err);
        const error = new HttpError(
            req,
...
//     let data;
//     try {
//         data = await standOutCourses.getOneCourse({ "standout_courses.id": id });
//         if (!data) {
//             const error = new HttpError(
//                 req,
//                 new Error().stack.split("at ")[1].trim(),
...
exports.getAllStandoutLessonsByCourses = async (req, res, next) => {
    ...
    try {
        list = await standOutModal.getAllLessons({
            match: {},
            page,
            per_page,
            sort_by,
...
exports.deleteStandoutLesson = async (req,res,next) =>{
    ...
    try {
        await standOutModal.deleteLesson({ id });
    } catch (err) {
        console.log("err : ",err);
        const error = new HttpError(
            req,
...
exports.getOneLessonForAdmin = async (req,res,next) => {
    ...
    let data;
    try {
        data = await standOutModal.getOneLessonAdmin({ "standout_lessons.id": id });
        if (!data) {
            const error = new HttpError(
                req,
                new Error().stack.split("at ")[1].trim(),
...
exports.createCategory = async (req, res, next) => {
    ...
    try {
        await standOutModal.createCategory(updates);
    } catch (err) {
        const error = new HttpError(
            req,
            new Error().stack.split("at ")[1].trim(),
...
exports.updateCategory = async (req, res, next) => {
    ...
    try {
        await standOutModal.updateCategory({ id }, updates);
    } catch (err) {
        const error = new HttpError(
            req,
            new Error().stack.split("at ")[1].trim(),
...
exports.getCategoryCourses = async (req, res, next) => {
    ...
    try {
        [categories, progress, thirdCompleted] = await Promise.all([
            standOutModal.getCategoryCourses(packageId,req.userId ?? 0),
            User.getCourseProgress("standout_course_categories", req.userId),
            standOutModal.lessonCheckThirdCategoryIsCompleted(req.userId ?? 0),
        ]);

        course_package_details = await standOutModal.getOneCoursePackage({id: packageId})
    ...
    categories = categories.map((category) => {
        category.courses = category.standout_courses.map((course) => {
            const lesson_count = course.standout_lessons.length;
            const each_lesson_percentage = +(100 / course.standout_lessons.length).toFixed(2);

            let lesson_completed_count = course.standout_lessons.filter(
            ...
            if (lesson_count === lesson_completed_count) {
                ...
                completed_percentage = course.standout_lessons.reduce((acc, cv) => {
            ...
            // course.lessons = course.standout_lessons.map((lesson) => lesson.id);

            course.lessons = course?.standout_lessons?.reduce((finalLessons, lesson) => {
                if (lesson && lesson.id) {
...
exports.thirdCategory = async (req, res, next) => {
    ...
    try {
        isThirdCategoryCompleted = await standOutModal.lessonCheckThirdCategoryIsCompleted(
            req.userId
        );
    } catch (err) {
        const error = new HttpError(
...
exports.getAllStandoutCategory = async (req, res, next) => {
    ...
    try {
        list = await standOutModal.getManyCategories({
            match: {},
            page,
            per_page,
            sort_by,
...
exports.getOneStandoutCategory = async (req, res, next) => {
    ...
    let data;
    try {
        data = await standOutModal.getOneCategory({ id });
        if (!data) {
            const error = new HttpError(
                req,
                new Error().stack.split("at ")[1].trim(),
...
exports.deleteStandoutCategory = async (req, res, next) => {
    ...
    try {
        await standOutModal.deleteCategory({ id });
    } catch (err) {
        console.log("err : ",err);
        const error = new HttpError(
            req,
...
exports.getAllStandoutPackage = async (req, res, next) => {
    ...
    try {
        list = await standOutModal.getAllCoursePackages({
            match: {},
            page,
            per_page,
            sort_by,
...
exports.getOneStandoutPackage = async (req, res, next) => {
    ...
    let data;
    try {
        data = await standOutModal.getOneStandoutPackage({ id });
        if (!data) {
            const error = new HttpError(
                req,
                new Error().stack.split("at ")[1].trim(),
...
exports.deleteStandoutPackage = async (req, res, next) => {
    ...
    try {
        await standOutModal.deleteStandoutPackage({ id });
    } catch (err) {
        console.log("err : ",err);
        const error = new HttpError(
            req,
...
exports.createStandoutPackage = async (req, res, next) => {
    ...
    try {
        await standOutModal.createStandoutPackage({
            title,
            description,
            image: course_image,
            created_at: dateTimeFormat(),
...
exports.updateStandoutPackage = async (req, res, next) => {
    ...
    try {
        await standOutModal.updateStandoutPackage({ id }, extras);
    } catch (err) {
        const error = new HttpError(
            req,
            new Error().stack.split("at ")[1].trim(),
...
exports.createCourse = async (req, res, next) => {
    ...
    try {
        await standOutModal.createCourse({
            package_id,
            category_id,
            title,
            description,
...
exports.updateCourse = async (req, res, next) => {
    ...
    try {
        await standOutModal.updateCourse({ id }, extras);
    } catch (err) {
        const error = new HttpError(
            req,
            new Error().stack.split("at ")[1].trim(),
...
exports.getOneCourse = async (req, res, next) => {
    ...
    try {
        course = await standOutModal.getOneCourse({ "standout_courses.id": id });
    } catch (err) {
        console.log(err);
        const error = new HttpError(
            req,
...
exports.getAllCourses = async (req, res, next) => {
    ...
    try {
        courses = await standOutModal.getAllCourses({
            page: 1,
            per_page: 100,
...
const getVideoLength = async (filename) => {
    ...
    const duration = await getVideoDuration(directoryPath);
...
exports.createLesson = async (req, res, next) => {
    ...
    try {
        await standOutModal.createLesson(updates);
    } catch (err) {
        const error = new HttpError(
            req,
            new Error().stack.split("at ")[1].trim(),
...
exports.updateLesson = async (req, res, next) => {
    ...
    try {
        await standOutModal.updateLesson({ id }, extras);
    } catch (err) {
        const error = new HttpError(
            req,
            new Error().stack.split("at ")[1].trim(),
...
exports.getOneLesson = async (req, res, next) => {
    ...
    try {
        lesson = await standOutModal.getOneLesson(
            { "standout_lessons.id": id },
            { "standout_lesson_progress.user_id": req.userId }
        );
    } catch (err) {
        console.log("err", err);
        const error = new HttpError(
    ...
    try {
        ...
        nextLesson = await standOutModal.lessonGetOneTwo({
            "standout_lessons.course_id": lesson.course_id,
            // "standout_lessons.order_number": lesson.order_number + 1,
        },[
            {key: "standout_lessons.order_number",operator: "=", value: lesson.order_number},
            {key: "standout_lessons.id",operator: "!=", value: lesson.id}
        ]);
        ...
        course = standOutModal.getOneCourse({ "standout_courses.id": lesson.course_id });

        progress = User.getCourseProgress("standout_course_categories", req.userId);
        ...
        nextCourse = await standOutModal.getNextCourse({category_id: course.category_id}, [{key: "standout_courses.order_number", operator: ">", value: course.order_number}])

        if (!nextCourse) {
            nextCourse = await standOutModal.getNextCourse({}, [
                {
                    key: 'category_id', operator: ">", value: course.category_id
                },
                {
                    key: "standout_courses.order_number", operator: ">", value: course.order_number
                }
            ])
        }
        ...
        if (nextCourse) {
            nextCourseLesson = standOutModal.lessonGetOneTwo({
                "standout_lessons.course_id": nextCourse.id,
            },[
                {key: "standout_lessons.order_number",operator: ">=", value: 1},
            ]);
    ...
    try {
        ...
        previousLesson = standOutModal.lessonGetOneTwo({
            "standout_lessons.course_id": lesson.course_id,
            // "standout_lessons.order_number": lesson.order_number + 1,
        },[
            {key: "standout_lessons.order_number",operator: "<=", value: lesson.order_number},
        ...
        if (!previousLesson) {
            previousLesson = await standOutModal.lessonRaw(
                "SELECT * FROM standout_lessons WHERE `course_id` = ? ORDER BY order_number DESC LIMIT 1",
                [lesson.course_id - 1]
            );
    ...
    if (
        ...
        const isExits = await standOutModal.getOneLessonProgress({
            user_id: req.userId,
            lesson_id: lesson.id,
            course_id: lesson.course_id,
        });
        ...
        if (!isExits) {
            if (lesson.is_assessment == 1) {
                const response = await standOutModal.createLessonProgress({
                    user_id: req.userId,
                    lesson_id: lesson.id,
                    course_id: lesson.course_id,
                    total_seconds: 1,
                ...
                await standOutModal.createLessonProgress({
                    user_id: req.userId,
                    lesson_id: lesson.id,
                    course_id: lesson.course_id,
                    total_seconds: 1,
    ...
    if (
        ...
        if (lesson.next_lesson_id && !lesson.is_another_course) {
            ...
            const nextLessonData = await standOutModal.lessonGetOneTwo({
                "standout_lessons.id": current_lesson_id,
            },{});
        ...
        await Promise.all([
            ...
            // standOutModal.createLessonProgress({
            //   user_id: req.userId,
            //   lesson_id: lesson.id,
            //   course_id: lesson.course_id,
            //   total_seconds: 1,
    ...
    try {
        if (course.category_id == 3) {
            const isThirdCategoryCompleted =
                await standOutModal.lessonCheckThirdCategoryIsCompleted(req.userId ?? 0);
...
exports.getAllLesson = async (req, res, next) => {
    ...
    try {
        lessons = await standOutModal.lessonGetMany({
            page: 1,
            per_page: 100,
            sort_by: "order_number",
            order: "asc",
...
exports.getAllLessons = async (req, res, next) => {
    ...
    try {
        lessons = standOutModal.lessonGetManyClient({
            page: 1,
            per_page: 100,
            sort_by: "order_number",
            order: "asc",
            match: {
                "standout_lessons.course_id": courseId,
            },
            matchTwo: {
                // "standout_lesson_progress.user_id": 2,
                user_id: req.userId ?? 0,
            },
        });

        course = standOutModal.getOneCourse({ "standout_courses.id": courseId });
...
exports.lessonUpdateId = async (req, res, next) => {
    ...
    for (let i = lastId; i >= firstId; i--) {
        await standOutModal.updateLessonProgress(
            {
                lesson_id: i,
            },
            {
...
exports.createLessonProgress = async (req, res, next) => {
    ...
    try {
        if (id) {
            ...
            response = await standOutModal.createLessonProgress(updates);
        }
    } catch (err) {
        const error = new HttpError(
            req,
...
exports.completeLessonProgress = async (req, res, next) => {
    ...
    try {
        ...
        response = await standOutModal.updateLessonProgress(
        ...
        const isCouseCompletedRes = await standOutModal.checkIsCourseIsCompleted({user_id,package_id})
        if(isCouseCompletedRes.status && isCouseCompletedRes.data){
            const packageDetails = await standOutModal.getOneCoursePackage({"id": package_id})
            console.log("packageDetails : ", packageDetails);
            await createCertificate(user_id, packageDetails.title, "standout_course")
        }
    } catch (err) {
    ...
    if (response.data > 0) {
        ...
        try {
            ...
            let progress = User.getCourseProgress("standout_course_categories", req.userId);
            ...
            if (lesson.id >= progress.current_lesson_id) {
                ...
                if (lesson.next_lesson_id && !lesson.is_another_course) {
                    ...
                    const nextLessonData = await standOutModal.lessonGetOneThree({
                        "standout_lessons.id": current_lesson_id,
                    });
