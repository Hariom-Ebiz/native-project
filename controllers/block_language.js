const Block = require("../models/block_language");
...
exports.create = async (req, res, next) => {
  const { page, title, description, block_id, language_id } = req.body;
  let extras = {
...
exports.update = async (req, res, next) => {
  const { id, page, title, description, language_id } = req.body;
  let extras = {
    updated_at: dateTimeFormat(),
  };
  let images = req.files;
...
//   extras.title_slug = createSlug(title);
  ...
  try {
    ...
    const getISItAvail = await Block.getOne({block_id: id, language_id});
    if (!getISItAvail) {
        updateRes = await Block.create(
            {
                block_id: id,
                language_id,
                title, 
        ...
        updateRes = await Block.update(
            { block_id: id, language_id },
            {
              title,
              description,
              ...extras,
...
exports.getOne = async (req, res, next) => {
  const { block_id, language_id } = req.params;

  let block;
  try {
    block = await Block.getOne({ block_id, language_id });
    if (!block) {
      const error = new HttpError(
        req,
        new Error().stack.split("at ")[1].trim(),
