...
const EmployerDetail = require("../models/employerDetails");
...
exports.create = async (req, res, next) => {
  ...
  message = message.replace(/\{LOGIN_LINK\}/g, `${process.env.FRONTEND_CANDIDATE_URL}`);
...
exports.employerCreate = async (req, res, next) => {
  const errors = validationResult(req);
  ...
  try {
    ...
    savedUser = await User.getOne({ email, user_role_id: 2 });
  } catch (err) {
  ...
  let message = emailTemplate.body;
  message = message.replace(/\{LOGIN_LINK\}/g, `${process.env.FRONTEND_EMPLOYER_URL}`);
  message = message.replace(/\{USER_NAME\}/g, first_name + " " + last_name);
  message = message.replace(/\{FIRST_NAME\}/g, first_name + " " + last_name);
  message = message.replace(/\{EMAIL\}/g, email);
  message = message.replace(/\{PASSWORD\}/g, password);
...
exports.getAll = async (req, res, next) => {
  ...
  sort_by = sort_by ?? "users.created_at";
  order = order ?? "desc";
...
exports.updateUser = async (req, res, next) => {
  ...
  res.status(200).json({
    status: true,
    message: "Job Hunter updated successfully.",
  });
};
...
exports.signup = async (req, res, next) => {
  console.log("normal sign up called");
  const errors = validationResult(req);
  ...
  try {
    ...
    savedUser = await User.getOne({ email, user_role_id: 1 });
  } catch (err) {
  ...
  message = message.replace(
    /\{LOGIN_LINK\}/g,
    `${process.env.FRONTEND_CANDIDATE_URL}/verify-email/${token}?role=1`
  );
...
exports.emailVerify = async (req, res, next) => {
  const { email, user_role_id } = req.body;

  if (!email && !user_role_id) {
    res.status(200).json({
      status: false,
    });
    return;
  ...
  let user;
  try {
    user = await User.getOne({ email, user_role_id });
...
exports.addMoreInfo = async (req, res, next) => {
  ...
  message = message.replace(
    /\{LOGIN_LINK\}/g,
    `${process.env.FRONTEND_CANDIDATE_URL}/login`
  );
  message = message.replace(
    /\{USER_NAME\}/g,
...
exports.resendVerificationToken = async (req, res, next) => {
  ...
  message = message.replace(
    /\{LOGIN_LINK\}/g,
    `${(user.user_role_id == "1") ? process.env.FRONTEND_CANDIDATE_URL : process.env.FRONTEND_EMPLOYER_URL}/verify-email/${token}?role=${user.user_role_id}`
  );
  message = message.replace(
    /\{NAME\}/g,
...
exports.jobSeekerLogin = async (req, res, next) => {
  ...
  let subscriptionStatus = {is_subscriber: null, was_subscriber: null}
  let subscriptions = {"career_coaching": 0, "standout": 0, "functional_mastery": []}
  try {
  ...
  if (!isEmailVerified) {
    ...
    let statusData = await Common.raw(`SELECT 
        CASE
            WHEN COUNT(*) > 0 THEN 1
            ELSE 0
        END AS was_subscriber,
        CASE
            WHEN COUNT(CASE WHEN is_expire = '0' THEN 1 END) > 0 THEN 1
            ELSE 0
    ...
    subscriptionStatus = statusData.data[0];

    let subscription = await Common.raw(`SELECT course_type, sub_course_type FROM candidate_subscriber WHERE job_seeker_id = '${userId}' AND is_expire = '0'`)
    for await (const i of subscription.data) {
      if (i.course_type == "career_coaching") {
        subscriptions["career_coaching"] = 1;
      } else if (i.course_type == "standout") {
        subscriptions["standout"] = 1
      } else if (i.course_type == "functional_mastery") {
        subscriptions.functional_mastery = [...subscriptions.functional_mastery, i.sub_course_type]
      }
    }
    ...
    res.status(200).json({
      ...
      is_subscriber: subscriptionStatus.is_subscriber || 0,
      was_subscriber: subscriptionStatus.was_subscriber || 0,
...
exports.jobSeekerForgotPassword = async (req, res, next) => {
  ...
  message = message.replace(
    ...
    `${(user_role_id == "1") ? process.env.FRONTEND_CANDIDATE_URL : process.env.FRONTEND_EMPLOYER_URL}/reset-password?token=${token}`
  );
...
exports.jobSeekerResetPassword = async (req, res, next) => {
  ...
  const { newPassword, token, user_role_id } = req.body;
...
exports.verify = async (req, res, next) => {
  ...
  if (role == 1) {
    try {
      ...
      let subscription = await Common.raw(`SELECT course_type, sub_course_type FROM candidate_subscriber WHERE job_seeker_id = '${id}' AND is_expire = '0'`)
      for await (const i of subscription.data) {
          if (i.course_type == "career_coaching") {
            subscriptions["career_coaching"] = 1;
          } else if(i.course_type == "standout") {
...
exports.employerSignup = async (req, res, next) => {
  ...
  message = message.replace(
    /\{LOGIN_LINK\}/g,
    `${process.env.FRONTEND_EMPLOYER_URL}/verify-email/${token}?role=2`
  );
  console.log("firstName ; ", first_name);
  message = message.replace(/\{NAME\}/g, first_name);
...
exports.EmployerLogin = async (req, res, next) => {
  ...
  let user, details, companyProfile, subscriptionStatus;
  try {
    user = await User.getOne({ email, user_role_id: 2, is_deleted: 0 });
    ...
    details = await EmployerDetail.getOne({ employer_id: user.id });
    companyProfile = await CompanyProfile.getOne({employer_id: user.id});

    const statusData = await Common.raw(`SELECT 
          CASE 
              WHEN COUNT(*) > 0 THEN 1
              ELSE 0
          END AS was_subscriber,
  ...
  if (!isEmailVerified) {
    ...
    try {
      ...
      message = message.replace(
        /\{LOGIN_LINK\}/g,
        `${process.env.FRONTEND_EMPLOYER_URL}/verify-email/${token}`
      );
...
exports.getCompanyProfile = async (req, res, next) => {
  ...
  let record = {};
  try {
    record = await User.getEmployerData(id);
  } catch (err) {
    const error = new HttpError(
      req,
      new Error().stack.split("at ")[1].trim(),
...
exports.getUserCertificates = async (req,res,next) => {
  try{    
    ...
    const certificates = await User.getUserCertificates(userId);
    res.status(200).json({
      status: true,
      message: "Fetched successfully.",
